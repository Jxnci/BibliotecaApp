  ...
  <div class="">
    ...
    <div class="flex justify-between  mb-3">
      <div class="flex items-center gap-x-2 text-base">
        ...
        <input type="text" class="rounded py-0.5 w-96" v-model="inputBuscar" @input="handleSearch('nombreApellido')">
        <span class="text-gray-800 dark:text-white ">Tipo</span>
        <select v-model="selectBuscar" @input="handleSearch('tipo')" class="rounded py-0.5">
    ...
    <div class="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody v-if="personas">
          <tr v-for="persona in personas" :key="persona.id" class="libro">
            <td class="px-3 py-1 text-sm font-medium text-gray-500 whitespace-nowrap">
              <h2 class="font-medium text-gray-800 dark:text-white ">{{ persona.nombres }} {{ persona.apellidos }}</h2>
              <p class="text-sm font-normal text-gray-600 dark:text-gray-400 -mt-1">{{ persona.tipo.descripcion }}</p>
            </td>
            <td class=" py-1 text-sm font-medium text-gray-500 whitespace-nowrap">{{ persona.celular }}</td>
    ...
    <div class="flex items-center justify-between mt-3">
      <button @click="fetchPersonas('','',currentPage - 1)" :disabled="currentPage === 1"
      ...
      <div class="items-center hidden lg:flex gap-x-3">
        ...
        <span v-if="personas.length === 0" class="animate-pulse bg-gray-50">
      ...
      <button @click="fetchPersonas('','',currentPage + 1)" :disabled="currentPage === lastPage"
        class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
        <span>Siguiente</span>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="w-5 h-5 rtl:-scale-x-100">
    ...
    <div class="sm:flex sm:space-x-6 text-gray-800">
      <div class="flex flex-col space-y-4">
        ...
        <div class="flex flex-col w-full">
          ...
          <input type="text" v-model="newItem.celular" class="rounded py-1">
        </div>
        <div class="flex flex-col w-full">
          <span class="text-sm text-gray-600 font-semibold">Tipo</span>
          <select v-model="newItem.tipo_id" class="rounded py-1">
...
const fetchPersonas = async (buscar, buscarParam, page = 1) => {
...
const handleSearch = async (input) => {
  ...
  const buscar = input === 'tipo' ? selectBuscar.value : inputBuscar.value;
  const buscarParam = input === 'tipo' ? 'tipo' : 'nombreApellido';

  await fetchPersonas(buscar, buscarParam);
  // } else {
  //   categorias.value = [];
  // }
};
